...
const AdminLayout = ({ children }: AdminLayoutProps) => {
  ...
  const [adminEmail, setAdminEmail] = useState<string>("admin@visiontrack.com");
    ...
    const checkUserRole = async () => {
      try {
        ...
        if (result.success && result.data.role === 'admin') {
          ...
          setAdminName(result.data.email.split('@')[0] || 'Admin User');
          setAdminEmail(result.data.email);
        } else if (result.success && result.data.role === 'employee') {
          // User is employee, not authorized
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <div className="w-10 h-10 bg-primary rounded-full flex items-center justify-center">
                  <span className="text-white font-semibold">{adminName?.[0] ?? 'A'}</span>
                </div>
                <div>
